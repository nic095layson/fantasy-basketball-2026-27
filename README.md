...
| [instructions/validation-prompts.md](instructions/validation-prompts.md) | **Validation suite** — paste-in prompts that confirm the Skills (claude-core-skills governors) and the operating procedures (the co-GM brief) are intact and firing, across a 2×2 matrix of **Opus/Sonnet × claude.ai/Cowork**. Each prompt baits a trap a degraded assistant walks into; PASS/FAIL criteria are behavior-based (verify/cite/flag/refuse), with a scorecard and stop-ship rule. Run before draft day and after any instruction/skill edit. |
