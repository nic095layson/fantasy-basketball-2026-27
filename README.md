...
| [SYSTEM-REVIEW.md](SYSTEM-REVIEW.md) | The independent review agent's master prompt — outsider-stance audit of the whole system (methodology, data, process, cross-plane coherence) against a championship 9-cat benchmark. Triages an owner-supplied weakness list, finds what the list missed, and designs prioritized fixes; proposes, never patches. Registered as the `system-critic` agent (`.claude/agents/system-critic.md`). |
