...
**Owner decisions, 2026-09-21 (in chat):** D51R-1/2/4 — proceed (deck PR #34).
D51R-3 — the punt advisor becomes advice-only until it earns trust. D51R-5 — the
Mkt column uses Yahoo's ADP, the room's price. S3 as F7 — the cross-plane gate
REFUSES to publish on a disagreement. D51R-6 — assists are a situational
casualty (only when winning confidently in a majority of the other categories);
Brook Lopez is faded (age, LAC situation) unless clearly the best left; the slate
is generated by `report/slate.py` (example in `report/seat-10-slate.md`).
D51R-7 — stands.
